...
## MCP-доступ для коллег (Kilo Code)

Скрипт-мост лежит в `mcp/firecrawl_mcp.py` (только stdlib, нужен `python3`).
Раздаётся через этот репозиторий; у каждого свой абсолютный путь в `command`.

Добавить в свой `kilo.jsonc` (или `~/.config/kilo/kilo.jsonc`) секцию:
...
```jsonc
{
  "mcp": {
    "firecrawl-stack": {
      "type": "local",
      "command": ["python3", "/АБСОЛЮТНЫЙ/ПУТЬ/до/firecrawl_mcp.py"],
      "environment": {
        "WORKER_URL": "http://109.94.1.210:3100/scrape",
        "FIRECRAWL_URL": "http://109.94.1.210:3002",
        "AUTH_TOKEN": "замените-на-реальный-токен"
      },
      "enabled": true
    }
  }
...
Затем `/mcps` (или перезапуск Kilo). Станут доступны тулы:

- `scrape_url(url, timeout)` — антидетект-воркер Camoufox (маркетплейсы, тяжёлые сайты).
- `scrape_markdown(url, timeout)` — Firecrawl (обычные сайты).

`AUTH_TOKEN` — общий ключ доступа к воркеру. Раздавай только своим; при утечке
смени на сервере (`sed -i "s/^AUTH_TOKEN=.*/AUTH_TOKEN=new/" .env && docker compose up -d --force-recreate camou-worker`).
Воркер — один браузер, запросы сериализуются: между вызовами выдерживай паузу.

### Сценарий «собрать информацию в интернете» (локальный харнесс)

Пакет автоматически направляет агента на MCP-стенд, когда сайт закрыт антиботом:

1. Пользователь: «собери информацию в интернете».
2. Агент (`deepseek` в локальном Kilo) открывает сайт через `webfetch`.
3. Сайт отдаёт капчу/блок — агент по скиллу `firecrawl-stack` вызывает
   `scrape_markdown` / `scrape_url` из MCP-сервера `firecrawl-stack`.
4. Данные получены в обход блокировки, агент отвечает пользователю.

Для этого у пользователя в проекте должен быть **скилл** `firecrawl-stack`:
скопировать папку `skill/firecrawl-stack` в `<проект>/.kilo/skill/` (в этом
репозитории он уже в `.kilo/skill/`). Скилл описывает, когда и какие тулы
MCP-сервера `firecrawl-stack` вызывать, и что делать при `captcha`/`ip-blocked`.
